...
//            sender.pubFirstData(tm.getDeviceId());
            ...
            while ((receivedLine = br.readLine()) != null) {
                ...
                // 拼接字符串
                jsonStringBuffer = jsonStringBuffer.append(new String(receivedLine));
                //用于解析json，7行一个json
                receivedLineNum++;
...
//                    sender.publish(json);
				...
				if (receivedLineNum % JSON_STRING_TIMETESE_LINE_SIZE == 0 
                    ...
                    Log.i(TAG, "[+] 成功从【jni层】接收到第" + receivedTimeTestJsonStringNum + "组【time test 数据】");
                }
