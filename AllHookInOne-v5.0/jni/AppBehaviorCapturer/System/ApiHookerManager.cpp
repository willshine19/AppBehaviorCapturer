...
bool ApiHookerManager::init(){
	LOGD("AppBehaviorCapturer system V4.0 is running");
//测试多态
	ApiHooker temp = *(new StartThreadApiHooker());
	temp.parseParameter();//此方法调用的是ApiHooker的parseParamter()方法
	ApiHooker *t1 = new StartThreadApiHooker();
	t1->parseParameter();//此方法调用的是StartThreadApiHooker的parseParamter()方法
	//获取系统发送模块的实例
...
bool ApiHookerManager::initHashMap(){
	...
	if(typeid(*start) == typeid(StartThreadApiHooker)){
		LOGD("start is StartThreadApiHooker1111111111111111111111111111111111111111");//输出结果是正确的
	}
