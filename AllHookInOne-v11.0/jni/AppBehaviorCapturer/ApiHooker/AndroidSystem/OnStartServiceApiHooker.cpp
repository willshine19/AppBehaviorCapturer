/*
 * OnStartServiceApiHooker.cpp
 *
 *  Created on: 2015-6-3
 *      Author: sang
 */

#include "OnStartServiceApiHooker.h"


OnStartServiceApiHooker::OnStartServiceApiHooker() {
	this->mApiDeclaration =
			*(new ApiDeclaration("android/app/Service", "onStart",
					"(Landroid/content/Intent;I)V", false, NULL, NULL, NULL));
...
/**
 * 该函数解析Service类的onStart方法的参数
 * args[0]表示调用主体，也就是Service（或其子类）实例
 * 调用实例的toString方法，将其信息打印到logcat
 */
bool OnStartServiceApiHooker::parseParameter(const u4* args) {
