/*
 * OnCreateApplicationApiHooker.cpp
 *
 *  Created on: 2015-5-12
 *      Author: long
 */

#include "OnCreateApplicationApiHooker.h"

OnCreateApplicationApiHooker::OnCreateApplicationApiHooker() {
	this->mApiDeclaration = *(new ApiDeclaration("android/app/Application",
...
/**
 * 该函数解析Application类的onCreate方法的参数
 * args[0]表示调用主体，也就是Application（或其子类）实例
 * 调用实例的toString方法，将其信息打印到logcat
 */
bool OnCreateApplicationApiHooker::parseParameter(const u4* args) {
	LOGD("[参数解析]开始解析oncreate方法的调用实例");
	JNIEnv *env = AndroidRuntime::getJNIEnv();
	jclass service = env->FindClass("android/app/Application");
	if (service == 0) {
	...
	LOGD("[+] application调用实例为 %s", paramString);
