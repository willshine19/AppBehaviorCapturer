...
pid_t find_pid_of(const char *process_name) {
	...
	while ((entry = readdir(dir)) != NULL) { //读目录
		id = atoi(entry->d_name); //如果第一个非空格字符存在，是数字或者正
		//负号则开始做类型转换，之后检测到非数字(包括结束符 \0) 字符时停止转换，返回整型数。
		if (id != 0) {
			sprintf(filename, "/proc/%d/cmdline", id);        //进程启动命令
			fp = fopen(filename, "r");
			if (fp) {
...
uint32_t find_dlclose_addr(pid_t pid) {
	...
	dlclose_addr = (uint32_t) get_remote_addr(pid, linker_path,
			(void*) dlclose);
...
uint32_t *inject_so_of(pid_t pid, const char *so_path) {
	...
	ptrace_writedata(pid, (void *) regs.ARM_sp, (void *) so_path,
			strlen(so_path) + 1);
	...
	ptrace_writedata(pid, (uint8_t *) saved_regs.ARM_sp, (uint8_t *) sbuf,
			sizeof(sbuf));
	ptrace_detach(pid);
	return (uint32_t *) r0;
}
...
uint32_t call_hook_fun(pid_t pid, void *handle, char *fun_name) {
	struct pt_regs regs;        //结构体描述了在执行系统调用时，用户态下的CPU寄存器在核心态的栈中的保存情况。
	memcpy(&regs, &saved_regs, sizeof(regs));
	ptrace_readdata(pid, (void *) regs.ARM_sp, (void *) sbuf, sizeof(sbuf)); //regs.ARM_sp:栈指针
	ptrace_writedata(pid, (void *) regs.ARM_sp, (void *) fun_name,
			strlen(fun_name) + 1);
	uint32_t parameters[2];
...
int main() {
	...
	// pid_t pid = find_pid_of("cn.m15.xys");
	// pid_t pid = find_pid_of("com.android.contacts");
	// pid_t pid = find_pid_of("sam.sucl.sqltest2");
	pid_t pid = find_pid_of("com.example.hookapi");
	ptrace_attach(pid);
	handle = inject_so_of(pid, "/data/local/wsn/libhook.so");
	ptrace_attach(pid);
	hookfun_addr = call_hook_fun(pid, handle, "handle_hook");
	DPRINTF("[+4]\thook成功！\n");
}
