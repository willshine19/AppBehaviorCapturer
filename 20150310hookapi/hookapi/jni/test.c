...
jstring Java_com_example_hookapi_MainActivity_stringFromJNI(JNIEnv* env, jobject thiz, jstring jstr) {
	...
	//open
	fdopen = open("/data/data/com.example.hookapi/files/s.txt",O_RDWR);
	if(fdopen==-1)
	...
	if(fdwrite==-1)
		...
		LOGI("s.txt write SUCCESS!!!  fdwrite=%d, string=%s",fdwrite,string);
