...
/**
 * 提供父进程观察和控制另一个进程执行的机制，同时提
 * 供查询和修改另一进程的核心影像与寄存器的能力。主
 * 要用于执行断点调试和系统调用跟踪。父进程可通过调用
 * fork ，接着指定所产生的子进程的 PTRACE_TRACEME 行为，
 * 最后使用 exec 等操作来初始化一个进程跟踪。
 * 绑定目标进程
 */
 ...
 */
int ptrace_call(pid_t pid, uint32_t addr, uint32_t *params, int num_params,
		struct pt_regs *regs) {
	int i;
