...
//					LOGD("sum_timeval_seconds is %ud",sum_timeval_seconds);
					ss = timeUtils->timevalToString(&timeUtils->time_subtract[t]);
					//保存时间差值对应的string数组
			...
			time_test["t2_t1_subtract_time"] = t2;
			//将avg(t2-t1)存入json
...
/*
			//发送t2
			for(int i = 0;i < CESHI_NUMBER;i++){
				len = timeUtils->t2_end_handle_string[i].size();
				int result = (int)send(sockfd, timeUtils->t2_end_handle_string[i].c_str(), len, 0);
				LOGD("[time test] t2 : %s",timeUtils->t2_end_handle_string[i].c_str());
//				usleep(2000);
			}
			//发送t2-t1
			for(int i = 0;i < CESHI_NUMBER;i++){
				len = timeUtils->time_subtract_string[i].size();
				int result = (int)send(sockfd, timeUtils->time_subtract_string[i].c_str(), len, 0);
				LOGD("[time test] t2-t1 : %s",timeUtils->time_subtract_string[i].c_str());
//				usleep(1000);
			}
			//发送t2-t1的平均值
			len = timeUtils->avg_time_subtract_string.size();
			int result = (int)send(sockfd,timeUtils->avg_time_subtract_string.c_str(),len,0);
			LOGD("[time test] avg(t2-t1) : %s",timeUtils->avg_time_subtract_string.c_str());
//			while(1);
*/
